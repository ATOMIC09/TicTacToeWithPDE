...
// สร้าง Object จากคลาสต่าง ๆ (Create Object from different classes)
...
class DrawAnimation{
    ...
    void drawX(){ // วาด X
        ...
        if (frameX >= 50){ // เมื่อวาดเสร็จแล้ว (If the drawing is complete)
            ...
            controller.checkWin();
            controller.checkTie();
            controller.playerFlip();
            isClicking = false;
            drawLoadArrayToGame = false;
            frameX = 0;
            frameY = 0;
            controller.checkHint();
        }
    }
...
void setup(){
    ...
    view.drawGrid(); // Draw grid
    view.drawSaveButton(); // Draw save button
    view.drawLoadButton(); // Draw load button
    view.drawResetButton(); // Draw reset button
    view.drawHintDisableButton(); // Draw hint disable button
    controller.resetArray(); // Reset array to default
}
    ...
    if (gameState == 0){
        ...
        if (isClicking == true){
            ...
            if (mouseX > 250 && mouseX < 350 && mouseY > 543 && mouseY < 611){
                ...
                if (isHint == false){
                    view.drawHintEnableButton();
                    isHint = true;
                }
                else{
                    view.drawHintDisableButton();
                    isHint = false;
                }
